...
async function launchCurrentQuestion(io: Server, accessCode: string) {
  ...
  setTimeout(async () => {
    try {
      ...
      setTimeout(async () => {
        try {
          const latestQuestion = await prisma.question.findUnique({
            where: { id: question.id },
            select: {
              status: true,
            },
          ...
          if (latestQuestion?.status !== "ACTIVE") return;
          ...
          console.error("launchCurrentQuestion results timeout failed", error);
        }
      }, timeLimit * 1000);
    } catch (error) {
      console.error("launchCurrentQuestion countdown timeout failed", error);
    }
  }, COUNTDOWN);
}
...
export function registerQuestionSockets(io: Server, socket: Socket) {
  ...
  socket.on("launch-question", async () => {
    try {
      ...
      await launchCurrentQuestion(io, accessCode);
    } catch (error) {
